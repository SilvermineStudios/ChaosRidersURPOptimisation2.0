...
public class WheelMultiplayer : MonoBehaviour
{
    #region Variables
    ...
    private void Update()
    {
        if (skid != null && Mathf.Abs(slipRatio) > 0.2)
        {
            ...
            //Debug.Log(2222);
            Vector3 skidPoint = new Vector3(transform.position.x, lasthit.point.y, transform.position.z) + (body.velocity * (Time.time - lastFixedUpdateTime));
            lastSkid = skidmarksController.AddSkidMark(skidPoint, lasthit.normal, 0.5f, lastSkid);
        }
        else
        {
            lastSkid = -1;
